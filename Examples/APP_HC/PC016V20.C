...
void PC016V20_Server(void)
{
	...
	if(SYSTIME>=1500)
	{
		SYSTime.TimeSYS		=	0;		//系统计时器
		SYSTime.TimeSEG		=	0;		//数码管更新数据计时器----可用于检测数码管通讯状态 20ms外发一个数码管状态
		SYSTime.TimeBOX		=	0;		//药箱检测计时器	每个端口分配100mS时间检测----可用于检测读卡器通讯状态
		SYSTime.TimeBUS		=	0;		//外部总线时间
		SYSTime.TimeCard	=	0;		//读卡器时间
...
//	BusFrameAck(&sFarmeRev);										//外部总线应答
	...
	else if((sFarmeRev.Head==0x7E)&&(((sFarmeRev.Cmd==0x13)&&(sFarmeRev.dAdd ==SwitchData-1))))		//取药指令，本层负责开锁
	{
...
*注释				:	
*******************************************************************************/
void BusFrameProcessGetIDbac(sFarmeDef*	sFarme,sBoradDef*	sBorad)			//数据命令处理---获取卡号
{
	u8	Temp	=	0;
	PortDef*	Port;
	if(sFarme->Addr3	==1)
...
//	Port->Cmd	=	sFarme->Cmd;
	...
	Temp=sFarme->dAdd;
	sFarme->dAdd	=	sFarme->sAdd;
	sFarme->sAdd	=	Temp;
	sFarme->Cmd		=	0x92;											//获取指定IC号上报
	
	if(Port->Status.BoxFlg	==	1)		//有卡
	{
		sFarme->data[0]=Port->CardNumber/100;			//百位
		sFarme->data[1]=Port->CardNumber%100/10;	//十位
		sFarme->data[2]=Port->CardNumber%10;			//个位
		sFarme->Length	=	4+3;
		sFarme->Bcc8	=	BCC8((u8*)sFarme->dAdd,10+3);		//8位数异或校验码：地址段+异常码+数据段校验
	}
	else if(Port->Status.BoxRead	==	1)//读卡器异常
	{
		sFarme->EC	=0x27;			//读卡器通讯异常
		sFarme->Length	=	4;
		sFarme->Bcc8	=	BCC8((u8*)sFarme->dAdd,10);		//8位数异或校验码：地址段+异常码+数据段校验
	}
	else
	{
		sFarme->EC	=0x23;			//无药箱数据
		sFarme->Length	=	4;
		sFarme->Bcc8	=	BCC8((u8*)sFarme->dAdd,10);		//8位数异或校验码：地址段+异常码+数据段校验
	}
	sFarme->End	=	0x7F;
...
//	Port->Cmd	=	sFarme->Cmd;				//命令
	Port->Ucode	=	sFarme->Ucode;		//用户码
	
	sFarme->Cmd	=	0x93;							//开锁取药箱
	sFarme->data[0]=Port->CardNumber/100;			//百位
	sFarme->data[1]=Port->CardNumber%100/10;	//十位
	sFarme->data[2]=Port->CardNumber%10;			//个位
	sFarme->Length	=	4+3;
	
	memcpy((u8*)RS485BusTxd,(u8*)sFarme,10);
	memcpy((u8*)&RS485BusTxd[10],(u8*)sFarme->data,3);
	RS485BusTxd[13]	=	BCC8((u8*)&RS485BusTxd[1],12);		//8位数异或校验码：地址段+异常码+数据段校验
	RS485BusTxd[14]	=	0x7F;
	RS485_DMASend(&RS485_Bus,(u32*)RS485BusTxd,15);	//RS485-DMA发送程序
}
/*******************************************************************************
*函数名			:	function
*功能描述		:	function
...
void BoardCardReaderReadAll(sBoradDef*	sBorad)							//向相应端口发送读卡指令
{
	...
	{
		SYSTime.TimeCard	++;							//读卡器时间
		if(SYSTime.TimeCard==15)					//等待15ms后开始，由于数码管串口端口与读卡器复用，需要等待
		{
			HALUsartRemapDisable();					//关闭串口复用
		}
		else if(SYSTime.TimeCard>=20)
		{
...
{
		SYSTime.TimeCard	++;								//数码管更新数据计时器----可用于检测数码管通讯状态 20ms外发一个数码管状态
		if(SYSTime.TimeCard==15)			//等待15ms后开始，由于数码管串口端口与读卡器复用，需要等待
		{
			HALUsartRemapDisable();					//关闭串口复用
		}
		if(SYSTime.TimeCard>1000)
		{
			return;
		}
		if((SYSTime.TimeCard%1000)==250)
		{
			return;
		}
		if((SYSTime.TimeCard%1000)	==	10)
		{
	//		HALUsartRemapDisable();												//关闭串口复用
	...
	//		GPIO_PinRemapConfig(GPIO_Remap_USART1,DISABLE);				//I/O口重映射开启
		...
		else if((SYSTime.TimeCard%1000)	==	50)
		{
			USART_DMASend	(ICCardReadPort1,(u32*)CardReaderCmd_ReadData,14);	//串口DMA发送程序
		}
		else if((SYSTime.TimeCard%1000)	==	100)
		{
			HALUsartRemapDisable();												//关闭串口复用
			
			USART_DMASend	(ICCardReadPort2,(u32*)CardReaderCmd_ReadData,14);	//串口DMA发送程序
		}
		else if((SYSTime.TimeCard%1000)	==	150)
		{
			USART_DMASend	(ICCardReadPort3,(u32*)CardReaderCmd_ReadData,14);	//串口DMA发送程序
		}
		else if((SYSTime.TimeCard%1000)	==	190)
		{
...
void BoardSaveCardData(sBoradDef*	sBorad)		//根据状态保存卡号及设置相关状态
{
	...
	{
		if(
				(Port->Status.RecData==1)		//bit7：0-无接收到卡数据，1-有接收到卡数据
			&&(Port->Status.BoxFlg==0))		//bit7：0-无接收到卡数据，1-有接收到卡数据==清除标志
		{
...
void BoardClrCardData(sBoradDef*	sBorad)		//删除卡号数据及设置相关状态
{
	...
	for(Num=0;Num<MaxPortNum;Num++)
	{	
		if(Port->Status.ClrData)				//bit8：0-无操作，1-删除数据请求(药箱不在）需要删除数据及更新数码管
		{
			//========清除数据
			Port->CardNumber	=	0;
		...
		if((sBorad->Time.TimeSEG%20)	==	0)	//20mS更新一个：通过485接口
		{
			...
			for(Num=0;Num<MaxPortNum;Num++)
			{
				if(Port->Status.BoxSeg	==	1)	//bit4：0-无操作，1-需要更新数码管
				{
					...
					if(Port->Status.BoxFlg	==	1)		//bit0：0-无卡,1-有卡
					{
						if(Port->Status.BoxDel	==	1)	//bit6：0-无待处理操作，	1-移除药箱请求(数码管数值闪烁),闪烁时间最大MaxOnTime
						{
				...
				if(Num>=MaxPortNum-2)		//if(sBorad->Step.WriteSeg	==	0)		//数码管更新标志为未更新，需要更新
				{
...
//	u8 Num	=	0;
	...
	if(SYSTime.TimeSEG>=1000)
	{
		return;
	}
	if((SYSTime.TimeSEG%1000)==250)
	{
	...
	if((SYSTime.TimeSEG%1000)	==	50)
	{
		DesAddr	=	1;				//数码管地址
		Port	=	&(sBorad->Port1);
	}
	else if((SYSTime.TimeSEG%1000)	==	100)
	{
		DesAddr	=	2;				//数码管地址
		Port	=	&(sBorad->Port2);
	}
	else if((SYSTime.TimeSEG%1000)	==	150)
	{
		DesAddr	=	3;				//数码管地址
		Port	=	&(sBorad->Port3);
	}
	else if((SYSTime.TimeSEG%1000)	==	200)
	{
		DesAddr	=	4;				//数码管地址
		Port	=	&(sBorad->Port4);
	}
