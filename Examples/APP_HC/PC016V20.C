...
*注释				:	
*******************************************************************************/
void MessageAnalysis(sFarmeDef*	sFarme)				//解析消息
{
...
*注释				:	
*******************************************************************************/
void MessagePacket(sFarmeDef*	sFarme)			//打包消息
{
}
...
//				Port->sBus.sBusFarme.Ucode;			//数据帧到达底层后，回复的数据帧总是将其按原值返回。若数据帧是底层设备自主发出，则用户码设为0，因此规定工控主机不能将下行数据帧的用户码设为0。
				Port->sBus.sBusFarme.Cmd		=	eGetID;					//上位机主动获取ID请求应答命令
				if(Port->Status.BoxRead	==1)		//bit2：0-读卡器正常，1-读卡器通讯异常
				{
					Port->sBus.sBusFarme.EC	=	eReadErr;						//读卡器通讯异常
					Port->sBus.sBusFarme.Length	=	FixedAddrLenth;
				}
				else if(Port->sBox.BoxSts.BoxOn	==0)	//bit1：0-无药箱,		1-有药箱
				{
					Port->sBus.sBusFarme.EC	=	eNoData;						//无药箱数据
					Port->sBus.sBusFarme.Length	=	FixedAddrLenth;
				}
				else
				{
					Port->sBus.sBusFarme.EC	=	eNoErr;							//无错误
			...
			{
				//根据以上两条标志确定上报类型
				if(Port->sBus.BusqSts.GotBox	==1)	//bit3：0-无操作，	1-药箱已被取走
				{
...
//					sFarme->Ucode	=	Port->Ucode;			//数据帧到达底层后，回复的数据帧总是将其按原值返回。若数据帧是底层设备自主发出，则用户码设为0，因此规定工控主机不能将下行数据帧的用户码设为0。
					...
					Port->sBus.sBusFarme.EC			=	eBoxBack;					//药箱收回
					Port->sBus.sBusFarme.Length	=	FixedAddrLenth+FixedDataLenth;					
				}
				...
				else
				{
					...
					Port->sBus.sBusFarme.Ucode	=	0x00;								//数据帧到达底层后，回复的数据帧总是将其按原值返回。若数据帧是底层设备自主发出，则用户码设为0，因此规定工控主机不能将下行数据帧的用户码设为0。
					Port->sBus.sBusFarme.Cmd		=	eSendID;					//下位机主动上传ID号命令
					Port->sBus.sBusFarme.EC			=	0x00;							//药箱收回
					Port->sBus.sBusFarme.Length	=	FixedAddrLenth+FixedDataLenth;
				}				
				break;			//退出For
			}
			else if(Port->sBus.BusqSts.GotBox	==1)	//bit3：0-无操作，	1-药箱已被取走
			{
				//此标志不能清除，需要等待药箱收回
//				Port->sBus.BusqSts.GotBox	=	0;				//bit3：0-无操作，	1-药箱已被取走
//				Port->sBus.sBusFarme.Ucode	=	Port->Ucode;	//数据帧到达底层后，回复的数据帧总是将其按原值返回。若数据帧是底层设备自主发出，则用户码设为0，因此规定工控主机不能将下行数据帧的用户码设为0。
				Port->sBus.sBusFarme.Cmd		=	eLock;				//下位机主动上传ID号命令
				Port->sBus.sBusFarme.EC		=	eBoxOff;			//药箱取走
				Port->sBus.sBusFarme.Length	=	FixedAddrLenth;
				break;			//退出For
			}
			else if(Port->sBus.BusqSts.TakeBox	==1)	//bit4：0-无操作，	1-药箱被强制取
...
//				Port->sBus.sBusFarme.Ucode	=	0x00;					//数据帧到达底层后，回复的数据帧总是将其按原值返回。若数据帧是底层设备自主发出，则用户码设为0，因此规定工控主机不能将下行数据帧的用户码设为0。
				Port->sBus.sBusFarme.Cmd		=	eLock;			//下位机主动上传ID号命令
				Port->sBus.sBusFarme.EC		=	eBoxTake;			//药箱被强制取走
				Port->sBus.sBusFarme.Length	=	FixedAddrLenth;
				break;			//退出For
			}
			
			else if(Port->sBus.BusqSts.TimeOut	==1)	//bit6：0-无操作，	1-取药超时
			{	
				Port->sBus.BusqSts.TimeOut	=	0;				//bit10：0-无操作，	1-取药状态下取出药箱后需要上报药箱状态
//				Port->sBus.sBusFarme.Ucode	=	Port->Ucode;	//数据帧到达底层后，回复的数据帧总是将其按原值返回。若数据帧是底层设备自主发出，则用户码设为0，因此规定工控主机不能将下行数据帧的用户码设为0。
				Port->sBus.sBusFarme.Cmd		=	eLock;				//下位机主动上传ID号命令
				Port->sBus.sBusFarme.EC			=	eBoxTout;			//药箱收回
				Port->sBus.sBusFarme.Length	=	FixedAddrLenth+FixedDataLenth;
				break;			//退出For
			}
			if(Num<MaxPortNum-1)
	...
	Port->sBus.sBusFarme.SN	=	sBorad->Nserial;						//流水号
	Port->sBus.sBusFarme.Head		=	eHead;		//头标识
	Port->sBus.sBusFarme.dAdd		=	0x00;
	Port->sBus.sBusFarme.sAdd		=	SwitchData;
	Port->sBus.sBusFarme.Addr2	=	SwitchData;
	Port->sBus.sBusFarme.Addr3	=	Port->PortNum;						//槽地址（端口号）
	...
	Port->sBus.sBusFarme.End		=	eEnd;			//结束标识
