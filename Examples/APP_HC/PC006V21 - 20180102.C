...
#define	PWM_UpdataCount			2			//频率变化占计数个数，加减速时PWM_Updata个脉冲后更新一次输出频率
#define	PWM_RunUpCount			500		//加速/减速需要的步数
#define	MOTOR_TIMx				TIM1		//旋转电机所使用的定时器

#define	SteepPerWindow			2*2030				//一个窗口用到的驱动脉冲数
#define	MaxWindow		5					//最大窗口数
...
u8 Motor_Server(void)				//查询步进电机状态
{
	...
	if(StatusOfWindow==0	&&	RunToWindow==0)				//电机上电初始化
	{
		...
		SteepCount				=	0;		//运行步数计数
		SteepNeedToRun		=	MaxWindow*SteepPerWindow+PWM_RunUpCount;				//需要运行的总步数
		RunFrequency			=	2*MOTOR_PWM_Frequency;
		TIM_SetFreq(MOTOR_TIMx,RunFrequency);		//设定频率MOTOR_PWM_Frequency
	...
	else if(StatusOfWindow!=0	&&	RunToWindow==0	&&	CMDOfWindow!=0)		//电机在待机待机状态下有新的窗口命令
	{
		...
		else if(CMDOfWindow	>	StatusOfWindow)
		{
			...
			if(RunToWindow	==	1)
			{
				SteepNeedToRun		=	(RunToWindow-StatusOfWindow)*SteepPerWindow+PWM_RunUpCount;				//需要运行的总步数
			}
			else
			{
